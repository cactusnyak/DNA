  ...
  imports: [
    NestConfigModule.forRoot({
      ...
      validationSchema: Joi.object({
        ...
        YANDEX_DELIVERY_TIMEOUT_MS: Joi.number().integer().min(500).max(30000).default(5000),
        YANDEX_DELIVERY_QUOTE_TTL_SECONDS: Joi.number().integer().min(60).max(3600).default(600),
        ...
        YANDEX_EXPRESS_MODE: Joi.string().valid('mock', 'manager_test', 'production').default('mock'),
        YANDEX_EXPRESS_BASE_URL: Joi.string().uri({ scheme: ['https'] }).default('https://b2b.taxi.yandex.net'),
        YANDEX_EXPRESS_TOKEN: Joi.string().allow('').optional(),
        YANDEX_RUSSIA_ENABLED: Joi.boolean().default(true),
        YANDEX_RUSSIA_MODE: Joi.string().valid('mock', 'sandbox', 'production').default('mock'),
        YANDEX_RUSSIA_BASE_URL: Joi.string().uri({ scheme: ['https'] }).default('https://b2b.taxi.tst.yandex.net'),
