...
const SEED_ASSETS_DIR = resolve(MODULE_DIR, 'assets', 'products');
...
const connectionString =
  ...
  (process.argv.includes('--validate-only')
    ? 'postgresql://seed-validation:seed-validation@127.0.0.1:5432/seed-validation'
    : undefined);
...
function assertPathInside(basePath: string, candidatePath: string): void {
  ...
  if (
    ...
    resolve(basePath, pathFromBase) !== candidatePath
  ) {
    throw new Error(
      `Путь выходит за пределы разрешённой директории: ${candidatePath}`,
    );
  }
}
...
async function filesAreEqual(
  ...
  if (!(await fileExists(secondPath))) return false;
...
async function assertSupportedImageContent(
  filePath: string,
  extension: string,
): Promise<void> {
  ...
  if (!matches)
    throw new Error(
      `Содержимое изображения не соответствует расширению: ${filePath}`,
    );
}

async function syncImageFile(
  image: PreparedImage,
): Promise<'copied' | 'unchanged' | 'replaced'> {
  assertPathInside(SEED_ASSETS_DIR, image.sourcePath);
  assertPathInside(UPLOADS_DIR, image.destinationPath);
  await mkdir(UPLOADS_DIR, { recursive: true });
  if (await filesAreEqual(image.sourcePath, image.destinationPath))
    return 'unchanged';

  const existed = await fileExists(image.destinationPath);
  const temporaryPath = `${image.destinationPath}.seed-tmp-${process.pid}`;
  await copyFile(image.sourcePath, temporaryPath);
  await rename(temporaryPath, image.destinationPath);
...
async function prepareImages(params: {
  ...
  for (const image of [...params.product.images].sort(
    ...
    if (!['.jpg', '.jpeg', '.png', '.webp'].includes(extension)) {
      throw new Error(
        `${params.product.seedKey}: неподдерживаемый формат: ${image.localPath}`,
      );
    }

    if (
      image.localPath !== image.localPath.trim() ||
      image.localPath.includes('/') ||
      image.localPath.includes('\\')
    ) {
      throw new Error(
        `${params.product.seedKey}: недопустимый путь изображения: ${image.localPath}`,
      );
    }
    ...
    assertPathInside(SEED_ASSETS_DIR, sourcePath);
    ...
    await assertSupportedImageContent(sourcePath, extension);
...
async function replaceProductImages(params: {
  ...
  const currentByUrl = new Map<
    string,
    (typeof currentLinks)[number]['image']
  >();
  const duplicateImageIds: string[] = [];
  for (const link of currentLinks) {
    if (currentByUrl.has(link.image.url)) duplicateImageIds.push(link.imageId);
    else currentByUrl.set(link.image.url, link.image);
  }
  const desiredUrls = new Set(params.images.map((image) => image.publicUrl));
  const staleImageIds = [
    ...duplicateImageIds,
    ...currentLinks
      .filter((link) => !desiredUrls.has(link.image.url))
      .map((link) => link.imageId),
  ];

  await prisma.$transaction(async (transaction) => {
    for (const image of params.images) {
      const linkedImage = currentByUrl.get(image.publicUrl);
      const imageRecord = linkedImage
        ? await transaction.image.update({
            where: { id: linkedImage.id },
            data: { sortOrder: image.sortOrder, alt: params.title },
          })
      ...
      await transaction.productImage.upsert({
        ...
        create: { productId: params.productId, imageId: imageRecord.id },
        update: {},
      });
    }
    ...
    if (staleImageIds.length > 0) {
      await transaction.productImage.deleteMany({
        where: { productId: params.productId, imageId: { in: staleImageIds } },
      });
...
export type SeedProfile = 'dev' | 'staging' | 'production';
...
export async function runCatalogSeed(options: {
  ...
  console.log('');
  console.log(`Профиль: ${options.profile}`);
  console.log('Источник: prisma/seeds/shared/catalog-seed.ts');
  ...
  const sourceImageOwners = new Map<string, string>();
  ...
  for (const product of PRODUCTION_PRODUCTS) {
    try {
      ...
      const sortOrders = product.images.map((image) => image.sortOrder);
      const expectedSortOrders = product.images.map((_, index) => index);
      if (
        new Set(sortOrders).size !== sortOrders.length ||
        sortOrders.some((value, index) => value !== expectedSortOrders[index])
      ) {
        throw new Error(
          'sortOrder изображений должен быть уникальной последовательностью от 0',
        );
      }

      for (const image of product.images) {
        const owner = sourceImageOwners.get(image.localPath);
        if (owner && owner !== product.seedKey) {
          throw new Error(
            `файл ${image.localPath} уже используется товаром ${owner}`,
          );
        }
        sourceImageOwners.set(image.localPath, product.seedKey);
      }
  ...
  if (options.imagesOnly) {
    ...
    let unchanged = 0;
    for (const product of PRODUCTION_PRODUCTS) {
      for (const image of await prepareImages({ product })) {
        const result = await syncImageFile(image);
    ...
    console.log(
      `Синхронизация файлов завершена: новых ${copied}, заменено ${replaced}, без изменений ${unchanged}.`,
    );
    return;
  }
